...
const useSettings = (props: UseSettingsProps) => {
  ...
  const UpdateNotesShowingStatus = useMutation(
    api.retros.updateNotesShowingStatus,
  );
  const UpdateHighlightModeStatus = useMutation(
    api.retros.updateHighlightModeStatus,
  );
  const settingToHandler = (
    ...
    const mapping = {
      [settings.notesShowingStatus.key]: (
        _value: string,
        { notesShowingStatus, ...rest }: Settings,
      ): Settings => {
        const newStatus = {
          showing: "hidden",
          hidden: "showing",
        ...
        notesShowingStatus.value = newStatus[
          notesShowingStatus.value
        ] as NotesShowingStatus;
